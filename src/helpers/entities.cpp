...
namespace entities
{
	...
	std::list<player_data_t> m_players;
	std::list<target_info_t> m_targets;
...
// 	}
	...
	void get_hitboxes(c_base_player* local, c_base_player* player, studiohdr_t* hdr, const bool& on_screen, bool& in_smoke, bool& is_visible, entities::player_data_t* entity)
	{
		...
		for (int k = HITBOX_HEAD; k < HITBOX_MAX; k++)
		{
			...
			auto& hitbox_pos = entity->hitboxes[k] = { mod, min, max, { (min + max) * 0.5f } };

			if (is_hitbox_has_multipoints(k))
			{
				math::vector2angles(hitbox_pos.points[0] - eye_pos, aim_angles);
				...
				if (k == HITBOX_HEAD)
					hitbox_pos.points[1] = hitbox_pos.points[0] + top * (mod * custom_scale);

				hitbox_pos.points[index] = hitbox_pos.points[0] + right * (mod * custom_scale);
				hitbox_pos.points[index + 1] = hitbox_pos.points[0] + left * (mod * custom_scale);
			}
			...
			if (!is_visible)
			{
				ray.Init(eye_pos, hitbox_pos.points[0]);
				g::engine_trace->trace_ray(ray, CONTENTS_SOLID | CONTENTS_MOVEABLE | CONTENTS_MONSTER | CONTENTS_DEBRIS | CONTENTS_HITBOX, &filter, &tr);

				is_visible = tr.endpos.DistTo(eye_pos) == hitbox_pos.points[0].DistTo(eye_pos);
			}

			if (!utils::is_line_goes_through_smoke(eye_pos, hitbox_pos.points[0]))
				in_smoke = false;
		}
	}

	void get_hitboxes(c_base_player* local, c_base_player* player, studiohdr_t* hdr, const bool& on_screen, bool& in_smoke, bool& is_visible, entities::player_data_t* entity, entities::target_info_t* target)
	{
		GetBBox(player, entity->points);
		...
		for (int k = HITBOX_HEAD; k < HITBOX_MAX; k++)
		{
			...
			auto& hitbox_pos = target->hitboxes[k] = { mod, min, max, { (min + max) * 0.5f } };
	...
	void fetch_aimbot_target_info(CUserCmd* cmd)
	{
		if (!g::global_vars)
			return;
		...
		const auto& max_clients = g::global_vars->maxClients;
		const auto& local_index = g::local_player->EntIndex();
		const auto& local_team = g::local_player->m_iTeamNum();
		...
		for (int i = 1; i <= max_clients; i++)
		{
			...
			const int& entity_index = player->EntIndex();

			if (entity_index == local_index)
				continue;

			const bool& is_player = player->IsPlayer();
			const bool& is_dormant = player->IsDormant();
			const bool& is_alive = player->IsAlive();
			const int& team_num = player->m_iTeamNum();

			if (!is_player || is_dormant || !is_alive)
				continue;

			const auto is_enemy = team_num != local_team;
			...
			target_info.abs_angles = player->GetAbsAngles();
			target_info.abs_origin = player->GetAbsOrigin();
			target_info.entity_index = entity_index;
			target_info.health = player->m_iHealth();
			target_info.is_alive = is_alive;
			target_info.is_dormant = is_dormant;
			target_info.is_enemy = is_enemy;
			target_info.is_flashed = player->IsFlashed();
			target_info.is_in_smoke = true; //EDIT later
			target_info.sim_time = player->m_flSimulationTime();
			target_info.team_num = team_num;
			target_info.eye_pos = player->GetEyePos();
			target_info.angles = player->m_angEyeAngles();
			...
			get_hitboxes(g::local_player, player, g::mdl_info->GetStudiomodel(player->GetModel()), on_screen, in_smoke, is_visible, &p, &target_info);
		}
	...
	void fetch_targets(CUserCmd* cmd)
	{
		...
		for (int i = 1; i <= max_clients; i++)
		{
			...
			// Removed (Reason: Modifying Netvar)
			...
			std::string temp_name = std::string(player->GetPlayerInfo().szName).substr(0, 12);
			size_t string_index = temp_name.find_last_not_of(" \t\n\r");

			if (string_index != std::string::npos)
				temp_name.substr(0, string_index + 1);


			player_data.name = temp_name;//std::string(player->GetPlayerInfo().szName).substr(0, 12);
			...
			get_hitboxes(local, player, studio_model, on_screen, in_smoke, is_visible, &player_data);
		...
		locker.lock();
		{
			m_players.emplace_back(player_data);
