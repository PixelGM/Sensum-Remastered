...
float scale(float initialNumber, float distance, float maxDistance, float minScale, float maxScale) {
	// Ensure non-negative distance and a valid maxDistance to avoid division by zero
	distance = std::max(0.0f, distance);
	maxDistance = std::max(1e-6f, maxDistance);

	// Calculate the scaling factor based on the distance between the objects
	float scalingFactor = (maxScale - minScale) * (1.0 - distance / maxDistance) + minScale;

	// Apply scaling to the initial number and return the result
	return initialNumber * scalingFactor;
}
...
float math::GetRealDistanceFOV(const float& distance, const QAngle& current, const QAngle& aim)
{
	...
	float new_fov = scale(fov, distance, 8192.f, 1.0f, 2.0f);

	if (settings::misc::print_fov)
		g::hud_chat->ChatPrintf(0, 0, std::format("fov: {:.1f}, new_fov: {:.1f}, distance: {:.1f}", fov, new_fov, distance).c_str());
